...
void setup() {
  ...
  mdns.begin(WiFi.localIP(), "arduino_motor1");

  char a1[] = "sensor1=/Pressure:0%100";
  char a2[] = "actuator1=/Motor:0%100";
  ...
  int success;
  success = mdns.addServiceRecord("Arduino with Motor 1._osc", 3333,
      MDNSServiceUDP, txt);
...
void loop() {
 ...
 if (client) {                             // if you get a client,
  ...
  if (server_ready == true) {
    ...
    //// Sending ////
    OSCMessage send_msg_pressure("/Pressure");
    send_msg_pressure.add(analogRead(Pressure) / 10);
  ...
  //// Recieving ////
    OSCMessage recieve_msg("/Pressure");
    ...
    if (udp_osc.remoteIP() == server_ip) {
      if (size > 0) {
        ...
        if (!recieve_msg.hasError()) {
          recieve_msg.dispatch("/Motor", motor);
